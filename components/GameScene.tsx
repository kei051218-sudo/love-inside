  ...
  phase2: {
    turn1: {
      ...
      defaultLine: {
        ...
        dukeun: '다정한 사람이라서 좋아했던 건데... 오늘은 좀 달랐어.',
    ...
    turn2: {
      ...
      choices: [
        { id: 'a2t2-vent',     text: '쌓인 감정을 털어놓는다', sub: '"사실 오늘만의 얘기가 아니야. 항상 나한테만 친절했으면 좋겠다고 했잖아."' },
        { id: 'a2t2-direct',   text: '직접 묻는다', sub: '"솔직히 얘기해봐. 오늘 내 친구가 눈에 들어온 거야?"' },
        { id: 'a2t2-indirect', text: '돌려서 묻는다', sub: '"우리 기념일인데 자기가 더 꾸미고 오냐구."' },
        { id: 'a2t2-giveup',   text: '오늘은 더 이상 얘기하지 않는다', sub: '"됐어. 오늘은 그냥 자자."' },
      ],
      choiceLines: {
        'a2t2-vent':     '사실 오늘만의 얘기가 아니야. 항상 나한테만 친절했으면 좋겠다고 했잖아.',
        'a2t2-direct':   '솔직히 얘기해봐. 오늘 내 친구가 눈에 들어온 거야?',
        'a2t2-indirect': '우리 기념일인데 자기가 더 꾸미고 오냐구.',
        'a2t2-giveup':   '됐어. 오늘은 그냥 자자.',
      },
    },
  },
  phase3: {
    turn1: {
      ...
      defaultLine: {
        ...
        dodo:   '오고 싶으면 오라고 해. 내가 먼저 나가지는 않을 거야.',
      },
      choices: [
...
const SCRIPT_B: Record<string, PhaseScript> = {
  ...
  phase2: {
    turn1: {
      lines: [
        { character: 'buni',   text: '오늘 날 최선을 다했어! 도대체 뭐가 문제인 거야! 서운하면 말을 하면 되는 거 아냐?' },
        ...
        { character: 'dukeun', text: '휴, 난 잘한다고 한 건데. 뭐가 그렇게 서운했던 걸까? 걱정돼.' },
      ],
      defaultLine: {
        buni:   '난 오늘 최선을 다했어. 뭐가 잘못이야?',
        seorup: '불편한 게 있었다면 말해줬으면 좋았을텐데.',
      ...
      choices: [
        ...
        { id: 'b2t1-worried',  text: '걱정돼', sub: '내가 무엇을 잘못했는지는 잘 모르겠지만, 네가 걱정돼.' },
      ],
    },
    turn2: {
      lines: [
...
const B_RESPONSES_A: Record<string, string> = {
  ...
  'a2t2-vent':        '...자기야. 그게 그렇게 힘들었어? 나 진짜 몰랐어. 미안해.',
  'a2t2-direct':      '...자기 친구한테 더 잘해준 것처럼 보였던 거야? 그런 뜻이 아니었는데. 자기 옆에 있으면서 딴 마음 품은 적 없어.',
  'a2t2-indirect':    '으응? 난 우리 자기밖에 눈에 안 들어와서 친구들은 뭐 입고 왔는지 기억도 안 나는데.',
  'a2t2-giveup':      '자기야, 우리 이러지 않기로 했잖아. 오늘 감정은 오늘 풀기로 약속했었잖아.',
...
const B_RESPONSES_B: Record<string, string> = {
  ...
  'b1t2-worry':     '...응. 집에 잘 들어왔어. 근데 걱정해줘서 고마워.',
  'b1t2-ask-why':   '...오늘 내 친구한테 새우도 까주고 한라봉까지 까줬잖아. 나는 바로 옆에 있었는데.',
  ...
  'b2t2-understand':'...그렇게 생각해줘서 고마워. 나도 말 못한 게 있었어.',
...
export default function GameScene({ player, onEnding }: Props) {
  ...
  const handleTurn2Choice = (choiceId: string) => {
    // choiceLines에서 실제 출력 대사 꺼내기
    const turn2Data = phase.turn2;
    const myText = turn2Data.choiceLines[choiceId] ?? turn2Data.defaultLine[player.emotion];
    const bText = B_RESPONSES[choiceId] ?? '...';

    // 엔딩 분기
    const undecidedIds = ['a2t2-giveup', 'a3t2-no-meet', 'b2t2-postpone', 'b3t2-no-go'];
  ...
  const handleBConfirm = () => {
    // 미결 엔딩
    if (['a2t2-giveup', 'a3t2-no-meet', 'b2t2-postpone', 'b3t2-no-go'].some(id =>
      bResponseText.includes(B_RESPONSES[id] ?? '___NONE___')
    )) {
      onEnding('undecided'); return;
    }
    if (phaseKey === 'phase5') {
      const happyTexts = ['미안해', '보고 싶었어', '안아준다', '속상했지', '나도 미안'];
      const isHappy = happyTexts.some(t => myLineText.includes(t));
      onEnding(isHappy ? 'happy' : 'breakup');
      return;
    }
